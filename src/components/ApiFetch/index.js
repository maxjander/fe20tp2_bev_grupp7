...
const ApiFetcher = (props) => {
    ...
    useEffect(() => {
        ...
        props.firebase.cards().once('value').then((snapshot) => { // catch a snapshot of the firebase database, but only once, which doesn't let it loop itself.
            ...
            for(k = 0; k < userBuyPoint.length; k++) // Loop which matches usercards to yugioh api, and checks for price difference, if there is one, pushes the delta to firebase
            {   
